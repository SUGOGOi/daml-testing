...
```bash
daml build
daml codegen js .daml/dist/my-daml-crud-app-0.1.0.dar -o ui/daml.js
cd ui
npm install
npm run-script build
zip -r ../my-daml-crud-app-ui.zip build
```

Next you need to create a ledger on [Daml Hub], upload the files
`.daml/dist/my-daml-crud-app-0.1.0.dar` (created by the `daml build` command)
and `my-daml-crud-app-ui.zip` (created by the `zip` command based on the result
